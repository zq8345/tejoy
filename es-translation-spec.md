...
|---|---|---|---|
| ① 人称 | `usted`（安全但冷） | ⭐ **`tú`** —— 墨西哥电商标准；`usted` 对 RV/房车这类消费品显得生分 | ⚠️ **候选·待墨西哥母语者确认** |
| ② 数字 | （悬着） | ⭐ **`25.6"` 小数点** · `1,200 Mbps` 逗号千分位 —— **跟美国一套，不是 `25,6"`** | ⚠️ **候选·待母语确认** |
| ③ 车 | `auto` | ⭐ **`carro`** —— 墨西哥本地说法；`auto` 能懂但不亲切 | ⚠️ **候选·待母语确认** |
| ④ `Case`→`Estuche` · `notebook`→`laptop` | ✅ | **保持** —— 墨西哥正是说 `laptop` | ⚠️ 同上 |
