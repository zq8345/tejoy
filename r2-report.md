...
## 11. alt 派生裁决(总工定)—— 依据我独立验过,成立;但**删字段有两个消费者要先处理**

### ✅ 依据独立复验:成立
| | 总工 | 我复验 |
|---|---|---|
| 标题重复(派生可无损重现)| 369 | **369**(前缀/截断)|
| 文件名(派生反而变好)| 59 | **59** |
| **真·独立描述(派生会损失)** | **0** | **0** ✅ |

→ **派生零损失,裁决成立。**

⚠️**我第一版判据用全等 `===` → 报"0 重复 / 369 真描述" → 差点用坏尺子推翻一个正确的裁决。** 实际 alt 是**标题的前缀截断**,前缀比对才是对的判据。
> **坏尺子不只会放行错的,还会否决对的。** 今天第 N 次。

### 🔴 删 JSON 里那 428 个 alt 之前:**有两个消费者**(总工问的"有没有我没想到的")
1. **`scripts/image-census.mjs`(多语言的工具)读 `im.alt`** —— 三处(`gallery`/`local`/`NO_SRC` 分支)。**删字段 = 打断别人的工具**,要先跟多语言对。
2. **`jsonld_product` 里含 alt 文本** —— 但 alt≈标题、jsonld 的 `name` 也是标题 → **极可能是标题重合而非真引用**。`"image"` 字段用的是 URL 不是 alt。**需单独确认再动。**

**→ 建议:先做派生(渲染层),`data` 里的 428 个 alt 暂留**;删字段单独一步,等这两个消费者确认后再做。**派生一上,d 的 738 就死了 —— 删不删数据不影响验收。**

### 实现要求(总工定)
- **显式优先**:Joe 在后台填了 alt → 用他的(后台 alt 输入框是他的地盘)
- 空/无 locale 版本 → `alt = i18n[locale].title`(派生,不存)
- ⚠️ **会改 en alt 输出**(59 个文件名 → 标题)→ **单独 commit,标「en 基线已移动 + 有意的内容改进」**
