...
## 10. ✅ R2 通过(总工重定线)+ `text` 259 的真相:**只剩 12 条,零新问题**

### 总工认了自己的线定错(我验过他的验证)
基线把 `a_cardTitles = 199` 记成一类,**实为两个东西**:
```
184 = 真·产品卡标题  ← R2 射程 → 184 → 0  ✅ 100%
 15 = 页面 <title>   ← 手写静态,R3 地界 → 仍 15(R2 够不着)
```
> 总工原话:「**我拿一个混合分类的数去定 a=0,这条线从第一天就不可能达到。不是你没做到,是我把靶子画在了两堵墙上。**」

**R2 合格线(最终)**:真卡片标题 **184→0** ✅ · `b_altSuffix` **256→0** ✅ · `e_links` **35→0** ✅ · en 零回归 **248/248** ✅ · 行内间距 **0 变动** ✅ → **通过**

### `text` 样本(总工问的 259)——**实测只有 12 条**
259 里的 **256 条正是我上轮修掉的裸标签**(`</label>Name` 那批)。剩 12 条**全部已有归属,零新问题**:

| 条数 | 内容 | 归属 |
|---|---|---|
| **5** | `Enterprise-Tejoy \| Premium Starlink…` `Performance (Gen 1)-…` 等 | **页面 `<title>`** → 已划 **R3**(存量,phase2 翻了一半漏了一半)|
| **4** | 葡语正文里的英文技术词:`Conversão Ethernet sem interrupções…` / `【XLinkShop】Para evitar perdas…` / `Chip inteligente e protocolo PD…` / `Compatibilidade específica com Starlink Mini…` | **扫描器命中葡语句子里的英文 token**(Ethernet/XLinkShop/PD/Starlink)—— **句子本身是葡语,非泄漏** |
| **3** | `Starlink 2M Router Cable`(657)· `Internet Kit Satellite`(680)· `power bank`(702) | **正是基线地板那 3 条** —— 真型号名 + 假阳性,**本就不该降** |

**→ `text` 里没有一条需要 R2 修。** d 的余量 = **738 alt(等 Joe)+ 64 placeholder(存量,待多语言签)+ 3 地板**。

### 剩余归属(都不是我的格子)
| 剩余 | 归属 |
|---|---|
| 15 页面 `<title>` | **R3** |
| 738 图库 alt | **结构缺口**(`images[].alt` 在 i18n 之外)—— **等 Joe 拍 alt 派生方案** |
| 64 company placeholder | **存量泄漏**(phase2 从没译过),已 token 化 + 留红 → **交多语言签** |
| 3 地板 | **本就不该降**(降了=型号被译坏)|
