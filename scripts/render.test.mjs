...
console.log("\n【④ 单跑 == 批量跑】 —— 结果依赖于「和谁一起跑」的工具,产出的是巧合:");
{
  ...
  // 清理:这些是测试造的产物,留在磁盘上 regen 会读到它们
  for (const f of fs.readdirSync("data/templates").filter((x) => /^page-/.test(x))) fs.unlinkSync(`data/templates/${f}`);
  for (const s of ["faq", "about", "video", "contact"]) { const p = `data/pages/${s}.json`; if (fs.existsSync(p)) fs.unlinkSync(p); }
}
